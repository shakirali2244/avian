exports.start = function start(socket){
    ...
    socket.on('completedExecutionFromPi', function(data){
        console.log('executed command' + data);
        io.sockets.emit('completedExecutionFromServerToClient', [JSON.parse(data)]);
    });
	...
	socket.on('takeoff',function (data){
		...
		if(data === 'default') {
			io.sockets.emit('commandFromServerToPi', { commandName: 'takeoff'});
		} else {
  ...
  //!TODO: Change this to commandNameFromPi -> commandNameFromServerToPi
  // Might be 
